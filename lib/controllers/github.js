...
module.exports = Router()
  ...
  .get('/callback', async (req, res) => {
    ...
    let existingUser = await GithubUser.findBylogin(login);
    
    if (!existingUser) {
      existingUser = await GithubUser.insert({ login, email, avatar: avatar_url });
    }
    const payload = jwt.sign({ ...existingUser }, process.env.JWT_SECRET, {
      expiresIn: '1 day',
    });
